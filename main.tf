...
terraform {
  ...
  backend "gcs" {
    bucket  = "devops-bucket-flask"
    prefix  = "terraform/state"
  }
}

resource "google_container_cluster" "primary" {
  name               = "flask-demo-k8s"  # cluster name
  ...
  provisioner "local-exec" {
    ...
    command = "gcloud container clusters get-credentials flask-demo-k8s --zone us-central1-c --project eng-skyline-420011"
  }
...
resource "google_compute_firewall" "nodeports" {
  ...
  allow {
    protocol = "tcp"
    ports    = ["30000-32767", "80", "443", "8080", "22", "5000"]  # valid ports in kubernetes is 30000-32767
  }
  allow {
    protocol = "icmp"
  }
